...
public class sensor_foreground extends Service implements SensorEventListener {
    ...
    public int onStartCommand(Intent intent,int flag,int start_id) {
        ...
        builder.setSmallIcon(R.drawable.ic_launcher_foreground).setTicker("HETT").setWhen(System.currentTimeMillis())
                .setNumber(0).setContentTitle("만보기").setContentText(Integer.toString(sensor.cnt)).setWhen(System.currentTimeMillis()).build();
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (gabOfTime > 100) {
                ...
                if (speed > SHAKE_THRESHOLD) {
                    sensor.cnt++;
                    sensor.tView.setText(Integer.toString(sensor.cnt));
                    builder.setContentText(Integer.toString(sensor.cnt));
                    Calendar calendar=Calendar.getInstance();
                    int month = calendar.get(Calendar.MONTH) + 1;
                    int day = calendar.get(Calendar.DAY_OF_MONTH);
                    if(!db.update(month,day,sensor.cnt))
                        db.insert(month,day,sensor.cnt);
                    startForeground(9999,builder.build());
                }
